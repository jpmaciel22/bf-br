...
(define (fold-funcs apl bf-funcs) ;; apl é a lista do array com pointer
...
(define-macro (bf-loop "[" OP-OR-LOOP-ARG ... "]") ;; iterar sobre o loop em si
  #'(lambda (arr ptr)
      (for/fold ([current-apl (list arr ptr)])
                ([i (in-naturals)]
                 #:break (zero? (apply current-byte current-apl)))
        (fold-funcs current-apl (list OP-OR-LOOP-ARG ...))))) ;; iterar as instrucoes
(provide bf-loop)
